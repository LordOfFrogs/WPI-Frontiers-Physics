...
wtext(text='<h1 style="color:SlateBlue">Buoyancy Simulation</h1><h3 style="color:SlateBlue">Tori Dell - Physics Frontiers 2023</h3>', pos=scene.title_anchor)
...
def drawBounds(len, rad):
    ...
    fluid = box(pos = vec(0, fluid_y/2-bounds_length/4, 0), size=vec(bounds_length, bounds_length/2+fluid_y, bounds_length), color=color.blue, opacity=0.2)
...
def start():
    ...
    for i in range(initNum):
        ...
        while not posSet:
            ...
            posSet=balls[i].pos.y-balls[i].radius > fluid_y
...
def calculateBuoyancy(ball):
    if ball.pos.y+ball.radius < fluid_y:
        vol = 4/3 * pi * ball.radius**3
        F = fluid_density * vol * g
        ball.vel.y += (F / ball.mass) * dt
        ball.vel *= 1 - fluid_friction*dt
    elif ball.pos.y-ball.radius < fluid_y:
        h = fluid_y-(ball.pos.y-ball.radius)
        vol = 1/3 * pi * h**2 * (3*ball.radius - h)
        F = fluid_density * vol * g
        ball.vel.y += F / ball.mass * dt
        ball.vel *= 1 - (2*pi*ball.radius*h / (4*pi*ball.radius**2)) * fluid_friction*dt
